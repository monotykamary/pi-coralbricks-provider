...
| `deprecated-models.json` | Graveyard for models the API delisted. update-models.js stamps them with deprecatedAt and pi keeps serving them for a 2-week grace period, then evicts them. A model Coral reports as RETIRED (404 `model_retired`) skips the grace period entirely — it cannot answer, so parking it only offers a dead model in `/model`. Never edit by hand. |
...
→ Run `node scripts/update-models.js`. Uses the authenticated `/v1/models` when a key is available (the `coralbricks` credential in auth.json or `CORALBRICKS_API_KEY`), otherwise falls back to the unauthenticated public catalog.
